...
class OpenAIAdapter(AiPort):
    ...
    def __init__(self, api_key: str, logger: ILoggingPort, config: AppConfig, model: str) -> None:
        self._client = AsyncOpenAI(api_key=api_key, timeout=httpx.Timeout(config.ai_model_connection_timeout))
        self._model = model
        self._logger = logger

    async def generate_reply(self, messages: tuple[Message, ...]) -> Message:
        self._logger.debug(f"Sending {len(messages)} messages to OpenAI model {self._model}.")
        api_messages = []
        ...
        try:
            ...
            self._logger.info("Successfully received reply from OpenAI.")
            return Message(role=MessageRole.ASSISTANT, content=reply_content)
        except OpenAIError as e:
            self._logger.error(f"OpenAI API error: {e}")
            return Message(
                role=MessageRole.ASSISTANT,
                content="I'm sorry, I'm having trouble connecting to GPT right now. Please try again in a moment.",
            )
        except Exception as e:
            self._logger.critical(f"An unexpected error occurred with the OpenAI adapter: {e}")
            return Message(
                role=MessageRole.ASSISTANT,
