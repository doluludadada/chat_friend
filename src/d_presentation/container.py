from dependency_injector import containers, providers
from src.b_application.configuration.schemas import AppConfig
from src.b_application.use_cases.conversation_usecase import ConversationUsecase
from src.c_infrastructure.ai_models.factory import AiAdapterFactory
from src.c_infrastructure.config.loader import load_settings
from src.c_infrastructure.persistence.inmemory_repository import InMemoryRepositoryAdapter
from src.c_infrastructure.platforms.line.line_adapter import LinePlatformAdapter
from src.c_infrastructure.platforms.line.line_handler import LineWebhookHandler
from src.c_infrastructure.platforms.line.line_security import LineSecurityService
from src.c_infrastructure.services.logger_service import LoggerService


class Container(containers.DeclarativeContainer):
    config: providers.Provider[AppConfig] = providers.Singleton(load_settings)
    logging = providers.Singleton(LoggerService, level=config.provided.log_level)

    ai_adapter_factory = providers.Factory(
        AiAdapterFactory,
        config=config,
        logger=logging,
    )
    ai_port = providers.Factory(lambda factory: factory.create_adapter(), factory=ai_adapter_factory)

    repository_port = providers.Singleton(InMemoryRepositoryAdapter, logger=logging)

    platform_port = providers.Factory(LinePlatformAdapter, config=config, logger=logging)

    conversation_usecase = providers.Factory(
        ConversationUsecase,
        ai_port=ai_port,
        repository_port=repository_port,
        platform_port=platform_port, 
        logging_port=logging,
    )

    line_security_service = providers.Factory(
        LineSecurityService,
        channel_secret=config.provided.line_channel_secret,
        logger=logging,
    )

    line_webhook_handler = providers.Factory(
        LineWebhookHandler,
        security_service=line_security_service,
        usecase=conversation_usecase,
    )
